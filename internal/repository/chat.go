...
func (r *chatRepository) CreateChat(ctx context.Context, create *model.CreateChat) (uuid.UUID, string, error) {
	...
	if create == nil {
		logger.Warn("Create data is empty")
	...
	// Validate chat type
	if create.Type != "dialog" && create.Type != "group" && create.Type != "channel" {
		logger.Warn("Invalid chat type provided")
	...
	// Build query (ID will be generated by database)
	query := `
...
func (r *chatRepository) GetUserRoleInChat(ctx context.Context, userID uuid.UUID, chatID uuid.UUID) (string, error) {
	var role string
	query := `SELECT user_role FROM user_chat WHERE user_id = $1 AND chat_id = $2`
